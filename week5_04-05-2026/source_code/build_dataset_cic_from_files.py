...
# ==========================================
# 1. FEATURE EXTRACTION & L7 DPI WEIGHTS
# ==========================================
...
# --- EMPIRICAL DNS RECORD TYPE WEIGHTING (Based on CIC Dataset) ---
def get_qtype_weight(qtype):
    """
    Weights derived from distribution variance (Information Gain) 
    between Malware and Benign traffic in CIC-DNS-EXF-2021.
    """
    if qtype == 12: return 3.25      # PTR: Highest anomaly in dataset
    elif qtype == 16: return 3.04    # TXT: High risk for payload exfiltration
    elif qtype == 255: return 2.98   # ANY: Often used to maximize response size
    elif qtype == 28: return 2.55    # AAAA: Moderate/High risk
    elif qtype == 33: return 2.05    # SRV: Moderate risk
    elif qtype == 1: return 0.02     # A: Standard benign traffic, extremely low risk
    
    # Default neutral weight for unseen record types
    return 1.0

# --- EXTRACT 5-DIMENSIONAL FEATURES ---
def extract_features(fqdn, qtype=1):
    fqdn = str(fqdn).strip()
    if fqdn.endswith('.'):
        fqdn = fqdn[:-1]
    ...
    # Feature 1 & 2: Length and Entropy
    ...
    # Feature 4: Record Type Weight (L7 DPI)
    type_weight = get_qtype_weight(qtype)

    # Feature 5: Consonant Ratio (Anti-Base64 Evasion)
            ...
            for pkt in packets:
                if pkt.haslayer(DNSQR) and pkt.haslayer(UDP):
                    ...
                    if label == 1:
                        ...
                        # Fixed
                        features = extract_features(query_name, pkt[DNSQR].qtype)
    ...
    try:
        with open(txt_path, "r", encoding="utf-8") as infile, open(out_file, "a", encoding="utf-8") as outfile:
            for line in infile:
                ...
                domain = line.strip()
                if domain:
                    features = extract_features(domain, 1) # Assume QTYPE 1 for domains.txt
