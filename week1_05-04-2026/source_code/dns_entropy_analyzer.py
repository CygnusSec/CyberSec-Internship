...
#TODO: PHASE 2 - INTEGRATION WITH DPI LOGS (SURICATA) FOR FREQUENCY
# Future updates will include tracking the frequency of queries over time.
# Logic to be implemented:
# 1. Parse 'eve.json' from Suricata to get Source IP and Timestamp.
# 2. Maintain a sliding window counter (e.g., max 50 queries/minute per IP).
# 3. Combine with current rules: IF (Frequency > Limit) OR (Length > 15 AND Entropy > 3.5) -> BLOCK.
